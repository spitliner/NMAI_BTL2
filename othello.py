...
class PvEBoard:
    ...
    def update(self):
        ...
        if not self.won:
            ...
            if self.player != self.humanTurn:
                startTime = time.time()
                self.oldarray = self.array
                alphaBetaResult = self.MNAB(self.array, depth, -float("inf"), float("inf"), 1)
    ...
    def GreedyMove(self):
        ...
        # Determines the best score based on the prior generated boards and a "Dumb" Heuristic: amountOfColour()
        bestScore = -float("inf")
        bestIndex = 0
        for i in range(len(boards)):
            score = amountOfColour(boards[i], self.player)
            if score > bestScore:
                bestIndex = i
                bestScore = score
        # Move to the best location based on amountOfColour()
        return choices[bestIndex][0], choices[bestIndex][1]
    ...
    def MNAB(self, node, depth, alpha, beta, maximizing):
        boards = []
        choices = []
        ...
        if maximizing:
            ...
            bestChoice = []
            for board in boards:
                boardValue = self.MNAB(board, depth - 1, alpha, beta, 0)[0]
                if boardValue > v:
                    v = boardValue
                    bestBoard = board
            ...
            bestChoice = []
            for board in boards:
                boardValue = self.MNAB(board, depth - 1, alpha, beta, 1)[0]
                if boardValue < v:
                    v = boardValue
                    bestBoard = board
                    bestChoice = choices[boards.index(board)]
...
def amountOfColour(array, player):
    score = 0
...
def finalHeuristic(array, player):
    ...
    elif moves <= 52 and checkCorner(array)<2:
        return decentHeuristic(array, player)
    elif moves <= 58 or checkCorner(array)<4:
        return (2*decentHeuristic(array, player) + 3*amountOfColour(array,player))/5

        # return slightlyLessDumbScore(array, player)
    else:
        return amountOfColour(array, player)
...
def runMenu():
    ...
    for i in range(3):
        ...
        for x in range(i + 1):
            ...
            screen.create_text(25 + (x + 1) * spacing + 155 * i, 326, anchor="center", text="\u2605",
                               font=("Consolas", 25), fill="#b29600")
            screen.create_text(25 + (x + 1) * spacing + 155 * i, 327, anchor="center", text="\u2605",
                               font=("Consolas", 25), fill="#b29600")
